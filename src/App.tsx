...
export default function DashboardPage() {
  ...
  useEffect(() => {
    const loadData = async () => {
      ...
      // 🛡️ HÀM GỌI API CHỐNG CRASH BẢO VỆ TOÀN DIỆN
  ...
  const widgetStats = useMemo(() => {
    const totalUnits = donViList.filter(dv => currentSubordinateIds.includes(dv.ID_DonVi) && dv.ID_DonVi !== selectedUnitFilter).length || 0;
  ...
  const staffChartData = useMemo(() => {
    ...
    nsData.forEach(ns => {
      // Lọc nhân sự thuộc cụm đang chọn và đang làm việc
        ...
        {loading ? (
          ...
          <div className="space-y-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-4 gap-4">
              ...
              {/* THẺ 2: TỔNG CHI NHÁNH / SR TRỰC THUỘC */}
