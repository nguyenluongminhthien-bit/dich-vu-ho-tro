...
export default function KhoaHocTab({ onReloadData, selectedUnitFilter, allowedDonViIds }: KhoaHocTabProps) {
  ...
  const handleValidateRow = (row: any) => {
    ...
    if (msnv) {
      ...
      if (!match) {
        ...
        const globalMatch = personnelList.find(p => p.ma_so_nhan_vien === msnv);
        if (globalMatch) {
          const dvName = donViList.find(d => String(d.id) === String(globalMatch.id_don_vi))?.ten_don_vi || 'Đơn vị khác';
          warnings['msnv'] = `MSNV thuộc đơn vị khác (${dvName}), không thuộc bộ lọc hiện tại.`;
        } else {
  ...
  const handleSavePastedHocVien = async (pastedRows: any[]) => {
    ...
    try {
      ...
      personnelList.forEach(p => {
        // Chỉ map nhân sự thuộc đơn vị đang lọc (activeUnitSubordinates)
        if (activeUnitSubordinates.includes(p.id_don_vi)) {
  ...
  const triggerBackgroundSync = async (hvToSync: any[]) => {
    ...
    const personnelMap = new Map<string, Personnel>();
    personnelList.forEach(p => {
      // Map theo key kết hợp "Mã_ĐơnVị" để tránh ghi đè chéo nếu trùng Mã nhân viên
      personnelMap.set(`${p.ma_so_nhan_vien}_${p.id_don_vi}`, p);
    });
