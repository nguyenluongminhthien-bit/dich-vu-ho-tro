...
import { X, Loader2, Save, Shield, Camera, Users, Link as LinkIcon, Compass, Clock, Sun, Moon, FileText, History } from 'lucide-react';
...
export default function SecurityModal({ isOpen, currentData, selectedUnitId, onSaved, onClose }: Props) {
  ...
  const displayedNccList = React.useMemo(() => {
    if (showAllNccGroups) return nccList;
    return nccList.filter(ncc => ncc.nhom_dich_vu === 'An ninh - Bảo vệ');
  }, [nccList, showAllNccGroups]);

  useEffect(() => {
    if (isOpen) {
      setError(null);
      setShowAllNccGroups(false);
      if (currentData) {
        setFormData({ ...currentData, id: currentData.id || '', id_ncc: currentData.id_ncc || null });
      } else {
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-4 gap-5">
                      <div className="sm:col-span-3">
                        <div className="flex justify-between items-center mb-1.5">
                          <label className="block text-xs font-bold text-gray-700">Nhà cung cấp BV</label>
                        ...
                        {formData.id_ncc === 'custom' ? (
                          <div className="relative">
                            <input
                              ...
                              className="w-full p-2.5 border border-gray-200 rounded-lg outline-none focus:border-[#05469B] transition-colors pr-10 font-semibold"
                            />
                            <button
                              type="button"
                              onClick={() => {
                          ...
                          <select
                            ...
                            className="w-full p-2.5 border border-gray-200 rounded-lg outline-none focus:border-[#05469B] transition-colors font-semibold text-gray-750"
                          >
                            <option value="">-- Chọn Nhà cung cấp --</option>
                            {displayedNccList.map(ncc => (
                              <option key={ncc.id} value={ncc.id}>
                                {ncc.ten_cong_ty} ({ncc.ten_goi_tat || 'Không có tên gọi tắt'})
                              </option>
                            ))}
