...
export const AllDocTable: React.FC<DocumentTableProps> = ({
  loading,
  paginatedDocs,
  filteredDocsCount,
  donViMap,
                      ...
                      {canEditOrDeleteDocument(item) ? (
                        <>
                          <button 
                            ...
                            className="p-1.5 text-[#05469B] hover:bg-blue-50 rounded-lg transition-colors" 
                        ...
                        <span className="text-[10px] text-gray-400 italic">Không có quyền sửa</span>
                      )}
                    </div>
                  </td>
                </tr>
      ...
      <div className="block md:hidden flex-1 overflow-y-auto pb-20 space-y-4">
        {loading ? (
          <div className="p-10 text-center">
            <Loader2 className="w-8 h-8 animate-spin mx-auto text-[#05469B] mb-2" /> 
            Đang tải...
          </div>
        ) : filteredDocsCount === 0 ? (
