            ...
            {loading ? (
              ...
              <tr>
                <td colSpan={7} className="p-16 text-center text-gray-500">
                  ...
                  <p className="text-lg font-medium">Không tìm thấy tờ trình nào phù hợp.</p>
                </td>
              </tr>
            ) : (
              paginatedDocs.map((item) => (
                ...
                >
                  ...
                  <td className="py-2.5 px-3">
                    ...
                    <p className="text-[9px] text-gray-400 font-bold uppercase mt-1">Từ: {donViMap[String(item.id_don_vi)] || item.id_don_vi}</p>
                  </td>
                  <td className="py-2.5 px-3 text-xs">
                    <div className="font-bold text-gray-800">{item.nguoi_ky ? normalizeSignerName(item.nguoi_ky) : '-'}</div>
                    {item.bo_phan_lay_so && (
                      <div className="text-[10px] text-gray-500 font-semibold mt-0.5">Trình: {item.bo_phan_lay_so}</div>
                    )}
                  </td>
                  <td className="py-2.5 px-3">
          ...
          paginatedDocs.map((item) => (
            ...
            >
              ...
              <div className="pl-2 mb-3">
                ...
                {(item.nguoi_ky || item.bo_phan_lay_so) && (
                  <div className="flex items-center gap-1.5 text-[10px] text-gray-500 font-semibold bg-gray-50/50 p-1.5 rounded-lg border border-gray-100 w-max">
                    ...
                    {item.bo_phan_lay_so && <span className="text-gray-400">(Trình: {item.bo_phan_lay_so})</span>}
                  </div>
                )}
              </div>
              ...
              <div className="pl-2 flex justify-between items-center text-[10px] text-gray-500 border-t border-gray-100 pt-3">
                ...
                <span className="truncate max-w-[50%] font-semibold text-right text-[#05469B]">
                  Từ: {donViMap[String(item.id_don_vi)] || item.id_don_vi}
                </span>
              </div>
