...
export default function CuocDiDongTab({
  ...
  }, [thueBaoList, cuocList, personnel, filterThang, filterPhapNhan, filterLoai, searchTerm, allowedDonViIds, selectedUnitFilter, selectedUnitSubordinates]);
  ...
  const tableRows = useMemo(() => {
    return baseRows.filter(row => {
      ...
      if (filterVuot === 'vuot') {
        ...
        if (row.tongCuoc === null || row.dinhMuc === null || (row.vuot !== null && row.vuot > 0)) return false;
      } else if (filterVuot === 'khong_dm') {
        if (row.dinhMuc !== null) return false;
      } else if (filterVuot === 'no_data') {
        if (row.tongCuoc !== null) return false;
      } else if (filterVuot === 'has_cuoc') {
        if (row.tongCuoc === null) return false;
      }
      return true;
    });
  }, [baseRows, filterTrangThai, filterVuot]);
  ...
  const stats = useMemo(() => {
    const totalSim = baseRows.length;
    const activeSim = baseRows.filter(r => r.tb.trang_thai === 'Đang hoạt động').length;
    const totalCuoc = baseRows.reduce((sum, r) => sum + (r.tongCuoc || 0), 0);
    const vuotCount = baseRows.filter(r => r.vuot !== null && r.vuot > 0).length;
    const trongCount = baseRows.filter(r => r.tongCuoc !== null && r.dinhMuc !== null && r.vuot !== null && r.vuot <= 0).length;
    const noDataCount = baseRows.filter(r => r.tongCuoc === null).length;
  ...
  return (
    <div className="flex flex-col flex-1 h-full min-h-0">
      ...
      <div className="grid grid-cols-2 md:grid-cols-6 gap-3 p-4 bg-gray-50 dark:bg-gray-900 border-b border-gray-200 dark:border-gray-800 shrink-0">
        <div 
          onClick={() => { setFilterTrangThai(''); setFilterVuot(''); setCurrentPage(1); }}
          className={`p-3 rounded-xl border flex items-center gap-3 shadow-xs cursor-pointer transition-all ${
            filterTrangThai === '' && filterVuot === '' 
              ? 'border-blue-500 bg-blue-50/20 ring-2 ring-blue-400 shadow-md' 
        ...
        <div 
          onClick={() => { setFilterTrangThai('Đang hoạt động'); setFilterVuot(''); setCurrentPage(1); }}
          className={`p-3 rounded-xl border flex items-center gap-3 shadow-xs cursor-pointer transition-all ${
            filterTrangThai === 'Đang hoạt động' && filterVuot === '' 
        ...
        <div 
          onClick={() => { setFilterTrangThai(''); setFilterVuot('has_cuoc'); setCurrentPage(1); }}
          className={`p-3 rounded-xl border flex items-center gap-3 shadow-xs cursor-pointer transition-all ${
            filterTrangThai === '' && filterVuot === 'has_cuoc' 
              ? 'border-indigo-500 bg-indigo-50/20 ring-2 ring-indigo-400 shadow-md' 
              : 'border-indigo-200 bg-white hover:border-indigo-500 hover:shadow-md'
          }`}
        >
        ...
        <div 
          onClick={() => { setFilterTrangThai(''); setFilterVuot('vuot'); setCurrentPage(1); }}
          className={`p-3 rounded-xl border flex items-center gap-3 shadow-xs cursor-pointer transition-all ${
            filterTrangThai === '' && filterVuot === 'vuot' 
        ...
        <div 
          onClick={() => { setFilterTrangThai(''); setFilterVuot('trong'); setCurrentPage(1); }}
          className={`p-3 rounded-xl border flex items-center gap-3 shadow-xs cursor-pointer transition-all ${
            filterTrangThai === '' && filterVuot === 'trong' 
              ? 'border-teal-500 bg-teal-50/20 ring-2 ring-teal-400 shadow-md' 
              : 'border-teal-200 bg-white hover:border-teal-500 hover:shadow-md'
          }`}
        >
        ...
        <div 
          onClick={() => { setFilterTrangThai(''); setFilterVuot('no_data'); setCurrentPage(1); }}
          className={`p-3 rounded-xl border flex items-center gap-3 shadow-xs cursor-pointer transition-all ${
            filterTrangThai === '' && filterVuot === 'no_data' 
              ? 'border-yellow-500 bg-yellow-50/20 ring-2 ring-yellow-400 shadow-md' 
              : 'border-yellow-250 bg-white hover:border-yellow-500 hover:shadow-md'
          }`}
        >
            ...
            <div className="flex-1 overflow-y-auto p-5 space-y-4 custom-scrollbar bg-white dark:bg-gray-800 text-sm">
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4 items-center">
                ...
                <div className="md:col-span-2 text-xs text-gray-600 dark:text-gray-300 leading-relaxed font-medium">
                  ...
                  <div className="bg-blue-50/50 dark:bg-blue-950/20 p-4 rounded-xl border border-blue-100 dark:border-blue-900/50 space-y-2.5">
                    ...
                    <div className="flex flex-wrap gap-1.5 text-[10px]">
                      <span className="px-2 py-0.5 rounded-md bg-white dark:bg-gray-900 border border-blue-300 dark:border-blue-700 text-blue-900 dark:text-blue-300 font-bold">1. Số điện thoại *</span>
                      <span className="px-2 py-0.5 rounded-md bg-white dark:bg-gray-900 border border-blue-200 dark:border-gray-800 text-gray-700 dark:text-gray-400 font-medium">2. Định mức</span>
                      <span className="px-2 py-0.5 rounded-md bg-white dark:bg-gray-900 border border-blue-200 dark:border-gray-800 text-gray-700 dark:text-gray-400 font-medium">3. Cước</span>
                      <span className="px-2 py-0.5 rounded-md bg-white dark:bg-gray-900 border border-blue-200 dark:border-gray-800 text-gray-700 dark:text-gray-400 font-medium">4. Khuyến mãi</span>
                      <span className="px-2 py-0.5 rounded-md bg-white dark:bg-gray-900 border border-blue-200 dark:border-gray-800 text-gray-700 dark:text-gray-400 font-medium">5. Thuế</span>
                      <span className="px-2 py-0.5 rounded-md bg-white dark:bg-gray-900 border border-blue-200 dark:border-gray-800 text-gray-700 dark:text-gray-400 font-medium">6. Điều chỉnh</span>
                      <span className="px-2 py-0.5 rounded-md bg-white dark:bg-gray-900 border border-blue-200 dark:border-gray-800 text-gray-700 dark:text-gray-400 font-medium">7. Nợ</span>
                      <span className="px-2 py-0.5 rounded-md bg-white dark:bg-gray-900 border border-blue-300 dark:border-blue-700 text-blue-900 dark:text-blue-300 font-bold">8. Cước sử dụng *</span>
                    </div>
                    <p className="text-[11px] text-gray-600 dark:text-gray-400 flex flex-wrap items-center gap-1">
                      📌 <strong className="text-gray-800 dark:text-gray-200">Ví dụ dòng dán từ Excel:</strong> 
                      <span className="font-mono bg-white dark:bg-gray-900 px-2 py-0.5 border border-gray-200 dark:border-gray-700 rounded text-indigo-700 dark:text-indigo-300">901806147 &nbsp;&nbsp; 200,000 &nbsp;&nbsp; 231,518 &nbsp;&nbsp; 34,728 &nbsp;&nbsp; 19,680 &nbsp;&nbsp; - &nbsp;&nbsp; - &nbsp;&nbsp; 216,470</span>
                    </p>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm p-4 animate-in fade-in duration-200">
          <div className="bg-white dark:bg-gray-800 rounded-2xl shadow-2xl w-full max-w-6xl max-h-[92vh] flex flex-col animate-in zoom-in duration-200 overflow-hidden border border-gray-200 dark:border-gray-700">
            ...
            <div className="overflow-y-auto flex-1 p-6 custom-scrollbar space-y-5">
              ...
              <div className="bg-indigo-50/50 dark:bg-indigo-950/20 p-5 rounded-2xl border border-indigo-100 dark:border-indigo-900/50 space-y-3">
                <div className="flex items-center gap-1.5 text-xs text-indigo-900 dark:text-indigo-300 font-bold">
                  <FileSpreadsheet size={16} />
                  <span>1. Cấu trúc dán (Quét từ Cột 1 đến Cột 8 trong Excel, có hay không có Tiêu đề đều được):</span>
                </div>
                <div className="flex flex-wrap gap-2 text-[10px]">
                  <span className="px-2.5 py-1 rounded-lg bg-white dark:bg-gray-900 border border-indigo-200 dark:border-gray-800 text-gray-500 font-medium">1. Số TT</span>
                  <span className="px-2.5 py-1 rounded-lg bg-white dark:bg-gray-900 border border-indigo-250 dark:border-gray-800 text-indigo-850 dark:text-indigo-400 font-medium">2. Mã NV</span>
                  <span className="px-2.5 py-1 rounded-lg bg-white dark:bg-gray-900 border border-indigo-350 dark:border-gray-700 text-indigo-900 dark:text-indigo-300 font-bold">3. Họ *</span>
                  <span className="px-2.5 py-1 rounded-lg bg-white dark:bg-gray-900 border border-indigo-350 dark:border-gray-700 text-indigo-900 dark:text-indigo-300 font-bold">4. Tên *</span>
                  <span className="px-2.5 py-1 rounded-lg bg-white dark:bg-gray-900 border border-indigo-200 dark:border-gray-800 text-gray-700 dark:text-gray-400 font-medium">5. Chức vụ - BP</span>
                  <span className="px-2.5 py-1 rounded-lg bg-white dark:bg-gray-900 border border-indigo-350 dark:border-gray-700 text-indigo-900 dark:text-indigo-300 font-bold">6. Số ĐT *</span>
                  <span className="px-2.5 py-1 rounded-lg bg-white dark:bg-gray-900 border border-indigo-200 dark:border-gray-800 text-gray-700 dark:text-gray-400 font-medium">7. Định mức</span>
                  <span className="px-2.5 py-1 rounded-lg bg-white dark:bg-gray-900 border border-indigo-200 dark:border-gray-800 text-gray-700 dark:text-gray-400 font-medium">8. Ngày cấp</span>
                </div>
                <div className="text-[11px] text-gray-600 dark:text-gray-400 flex flex-wrap items-center gap-1">
                  📌 <strong className="text-gray-800 dark:text-gray-200">Ví dụ dòng dán từ Excel:</strong>
                  <span className="font-mono bg-white dark:bg-gray-900 px-2 py-0.5 border border-gray-200 dark:border-gray-700 rounded text-indigo-700 dark:text-indigo-300">1 &nbsp;&nbsp; NV001 &nbsp;&nbsp; Nguyễn &nbsp;&nbsp; Văn A &nbsp;&nbsp; Chuyên viên &nbsp;&nbsp; 938903526 &nbsp;&nbsp; 200,000 &nbsp;&nbsp; 31/03/2016</span>
                </div>
              </div>

              <div className="space-y-1.5">
                <label className="block text-xs font-bold text-gray-700 dark:text-gray-300">Dữ liệu dán Excel (Ctrl + V)</label>
                <textarea
                  rows={4}
