...
interface ChartTooltipState {
  ...
  prevVal: number | null;
  diff: number | null;
  status: 'vượt' | 'trong' | 'chưa' | 'khong_gioi_han';
}
...
export default function PersonnelDetailCuocChart({ personnel }: Props) {
  ...
  const handleMouseMove = (e: React.MouseEvent<SVGRectElement>, item: typeof chartData[0]) => {
    ...
    let status: 'vượt' | 'trong' | 'chưa' | 'khong_gioi_han' = 'chưa';
    if (item.currVal !== null) {
      if (nguongDinhMuc === null) {
        status = 'khong_gioi_han';
      } else {
    ...
    <div className="flex flex-col h-full relative select-none">
      ...
      <div className="relative flex-1 w-full flex items-center justify-center min-h-[220px]">
        <svg viewBox={`0 0 ${svgWidth} ${svgHeight}`} className="w-full h-auto overflow-visible">
          ...
          {/* Đường định mức màu đỏ (Chỉ vẽ khi có định mức) */}
            ...
            <div className="space-y-1.5 font-medium text-[11.5px]">
              ...
              {tooltip.diff !== null && (
                <div className="flex justify-between items-center pt-1 border-t border-gray-100">
                  ...
                  <span
                    className={`font-black ${tooltip.diff > 0
              ...
              <div className="pt-1.5 mt-1 border-t border-gray-100 flex items-center justify-between">
                ...
                {tooltip.status === 'khong_gioi_han' && (
                  <span className="px-2 py-0.5 bg-blue-50 text-blue-700 font-extrabold rounded-md border border-blue-200 text-[10.5px]">
                    Không giới hạn
                  </span>
                )}
                {tooltip.status === 'vượt' && (
