...
export default function PersonnelModal({
  ...
  const handleGPLXChange = (value: string, isChecked: boolean) => {
    let currentArr = typeof formData.giay_phep_lai_xe === 'string'
      ? formData.giay_phep_lai_xe.split(',').map((s: string) => s.trim()).filter(Boolean)
      : [];
    if (value === 'Không có') {
  ...
  const currentGPLXList = useMemo(() => {
    return typeof formData.giay_phep_lai_xe === 'string'
      ? formData.giay_phep_lai_xe.split(',').map((s: string) => s.trim()).filter(Boolean)
      : [];
                ...
                <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
                  ...
                  <div className="col-span-1 md:col-span-4 border-t border-gray-100 mt-2 pt-4">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
                      ...
                      <div>
                        <label className="block text-xs font-bold text-gray-700 mb-1">Thẻ gửi xe (Mã thẻ)</label>
                        <input
                          type="text"
                          name="the_xe"
                          value={(formData.the_xe === 'No' || formData.the_xe === 'Không có') ? '' : (formData.the_xe || '')}
                          onChange={handleInputChange}
                          placeholder="Mã thẻ xe (nếu có)..."
                          className="w-full p-2.5 border border-gray-200 rounded-lg bg-[#FFFFF0] outline-none focus:ring-2 focus:ring-[#05469B]"
                        />
                      </div>
                      {Boolean(formData.the_xe && formData.the_xe !== 'No' && formData.the_xe !== 'Không có' && String(formData.the_xe).trim() !== '') && (
                        <>
                          <div>
                            ...
                            <input
                              type="text"
                              name="hang_loai_xe"
                              value={formData.hang_loai_xe || ''}
                              onChange={handleInputChange}
                              className="w-full p-2.5 border border-gray-200 rounded-lg bg-[#FFFFF0] outline-none focus:ring-2 focus:ring-[#05469B]"
                              placeholder="VD: Honda Airblade"
                            />
                          </div>
                          <div>
                  ...
                  return (
                    <label key={cert.id} className="flex items-center p-2.5 border border-emerald-200 rounded-lg bg-[#FFFFF0] cursor-pointer hover:border-emerald-500 transition-colors shadow-sm">
                      <input
                        ...
                        name={cert.id}
                        checked={
                          cert.id === 'giay_phep_lai_xe'
                            ? (formData.giay_phep_lai_xe !== undefined && formData.giay_phep_lai_xe !== null && formData.giay_phep_lai_xe !== 'Không có')
                            : (formData[cert.id] === true || String(formData[cert.id]).toLowerCase() === 'true')
                        }
                        onChange={handleInputChange}
