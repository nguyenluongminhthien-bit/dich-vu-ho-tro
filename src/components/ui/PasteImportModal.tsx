...
import { X, ClipboardPaste, AlertTriangle, CheckCircle, Loader2, Info, Trash2 } from 'lucide-react';
...
const getColMinWidth = (key: string): string => {
  switch (key) {
    case 'stt': return '20px';
    case 'msnv': return '60px';
    case 'ho_ten': return '150px';
    case 'ngay_sinh': return '75px';
    case 'gioi_tinh': return '50px';
    case 'so_cccd': return '80px';
    case 'quoc_tich': return '75px';
    case 'chuc_vu': return '90px';
    case 'don_vi_text': return '100px';
    case 'nhom': return '50px';
    case 'noi_dung_huan_luyen': return '180px';
    case 'thoi_gian_text': return '110px';
    case 'diem_ly_thuyet': return '50px';
    case 'diem_thuc_hanh': return '50px';
    case 'ket_qua': return '75px';
    case 'ghi_chu': return '100px';
...
export default function PasteImportModal({
  ...
  // Tính toán vị trí sticky left cho các cột
  const getStickyLeftStyle = (colKey: string): React.CSSProperties => {
    if (colKey === 'stt') {
      return { position: 'sticky', left: '50px', zIndex: 2 };
    }
    if (colKey === 'msnv') {
      const hasSttCol = columnMapping.some(c => c.key === 'stt');
      const leftVal = hasSttCol ? 100 : 50;
    ...
    if (colKey === 'ho_ten') {
      ...
      let leftVal = 50;
      if (hasSttCol) leftVal += 50;
      if (hasMsnvCol) leftVal += 90;
      return { position: 'sticky', left: `${leftVal}px`, zIndex: 2 };
    }
    return {};
  };
  ...
  const getStickyThStyle = (colKey: string): React.CSSProperties => {
    ...
    if (base.position === 'sticky') {
      return {
        ...base,
        zIndex: 12, // Cao hơn z-index của sticky header thường (10) và body cells (2)
  ...
  const handleDeleteRow = (indexToDelete: number) => {
    setParsedData(prev => prev.filter((_, idx) => idx !== indexToDelete));
    setRowStatus(prev => prev.filter((_, idx) => idx !== indexToDelete));
  };
          ...
          {!isAnalyzing && parsedData.length > 0 && (
            <div className="flex-1 flex flex-col gap-3 min-h-[250px]">
              ...
              <div className="flex-1 overflow-auto border border-gray-200 rounded-2xl">
                <table className="w-full text-left border-collapse text-[10px]" style={{ minWidth: '1850px' }}>
                  <thead className="bg-gray-50 sticky top-0 z-10 border-b border-gray-200">
                    <tr>
                      <th
                        className="py-1.5 px-2 font-bold text-gray-600 w-10 border-r border-gray-200 text-center"
                        style={{ minWidth: '50px', position: 'sticky', left: 0, zIndex: 12, backgroundColor: '#f9fafb' }}
                      >
                        STT
                      </th>
                      ...
                      <th className="py-1.5 px-2 font-bold text-gray-600 border-r border-gray-200 font-semibold" style={{ minWidth: '150px' }}>Trạng thái rà soát</th>
                      <th className="py-1.5 px-2 font-bold text-gray-600 w-12 text-center font-semibold" style={{ minWidth: '60px' }}>Hành động</th>
                    </tr>
                  </thead>
                  <tbody>
                    {parsedData.map((item, idx) => {
                      ...
                      return (
                        <tr key={idx} className={`group border-b border-gray-150 transition-colors ${rowClass}`}>
                          <td
                            className={`py-1.5 px-2 text-gray-400 font-semibold text-center border-r border-gray-200 ${stickyBgClass}`}
                            style={{ minWidth: '50px', position: 'sticky', left: 0, zIndex: 2 }}
                          >
                            {idx + 1}
                          </td>
                          {columnMapping.map(col => {
                            ...
                            let cellClass = 'border-r border-gray-250 p-0'; // Bỏ padding để input chiếm trọn ô
                            ...
                            // Format ngày hiển thị ban đầu trong ô nhập liệu (nếu có định dạng ISO hợp lệ thì chuyển sang dd/mm/yyyy để dễ sửa)
                            let displayVal = val ?? '';
                            ...
                            return (
                              ...
                              >
                                <input
                                  ...
                                  onBlur={(e) => handleCellBlur(idx, col.key, e.target.value)}
                          ...
                          <td className="py-1.5 px-2 border-r border-gray-200" style={{ minWidth: '150px' }}>
                            {rowHasError && (
