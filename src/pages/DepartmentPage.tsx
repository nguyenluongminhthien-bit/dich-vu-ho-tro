...
const initialFormState: Partial<DonVi> = {
  ...
  SoPhongCho: '', SoCong: '', LuotKhachBQ: '', TongNhanSu: '', loaiHinh: 'Showroom', trangThai: 'Hoạt động', Phia: 'CTTT Phía Nam'
};
...
export default function DepartmentPage() {
  ...
  const [anNinhData, setAnNinhData] = useState<AnNinh[]>([]);
  ...
  const [securityFormData, setSecurityFormData] = useState<Partial<AnNinh>>({});
  ...
  const selectedUnitSubordinates = useMemo(() => {
    ...
    const level1 = [selectedUnitId];
    const level2 = data.filter(dv => level1.includes(dv.CapQuanLy)).map(dv => dv.ID_DonVi);
    const level3 = data.filter(dv => level2.includes(dv.CapQuanLy)).map(dv => dv.ID_DonVi);
    return [...level1, ...level2, ...level3];
  }, [selectedUnitId, data]);

  const currentAnNinh = useMemo(() => anNinhData.find(item => item.ID_DonVi === selectedUnitId) || null, [anNinhData, selectedUnitId]);
  
  const currentPhapNhanList = useMemo(() => {
    return phapNhanData.filter(item => selectedUnitSubordinates.includes(item.ID_DonVi));
  ...
  const handleInputChange = (e: React.ChangeEvent<HTMLInputElement | HTMLTextAreaElement | HTMLSelectElement>, formType: 'pn' | 'ph' | 'sec' = 'pn') => {
  ...
  const openSecurityModal = () => {
    if (currentAnNinh) { setSecurityFormData({ ...currentAnNinh }); } 
    else {
      setSecurityFormData({
  ...
  const handleSecuritySave = async (e: React.FormEvent) => {
    ...
    let finalData = { ...securityFormData, TongANBV: soNB + soDV };
    if (soDV < 1) { finalData.VitrBVDV = ''; finalData.NCC_DichVu = ''; finalData.ChiPhiThue = ''; }

    try {
      const mode = finalData.ID_AnNinh ? 'update' : 'create';
      const res = await apiService.save(finalData, mode, "HS_AnNinh");
      if (mode === 'create') {
        finalData.ID_AnNinh = res.newId; setAnNinhData(prev => [...prev, finalData as AnNinh]);
      } else {
        setAnNinhData(prev => prev.map(item => item.ID_AnNinh === finalData.ID_AnNinh ? finalData as AnNinh : item));
      }
  ...
  return (
    <div className="flex h-full bg-[#f4f7f9] overflow-hidden relative">
      ...
      {/* CỘT PHẢI CHI TIẾT */}
      <div className="flex-1 overflow-y-auto p-6 relative transition-all duration-300">
        <div className={`max-w-6xl mx-auto space-y-8 pb-12 transition-all duration-300 ${isListCollapsed ? 'pl-12' : ''}`}>
          ...
          {!selectedUnit ? (
            ...
            <div className="animate-in fade-in duration-300 space-y-8">
              ...
              {/* Header Đơn vị */}
              <div className="flex justify-between items-start bg-white p-6 rounded-2xl shadow-sm border border-gray-200">
                <div>
                  ...
                  <div className="flex flex-wrap items-center gap-3 text-sm text-gray-500 font-medium">
                    <span className="flex items-center gap-1.5 group">
                      ...
                      {selectedUnit.DiaChi ? (
                        <a href={`http://maps.google.com/?q=${encodeURIComponent(selectedUnit.DiaChi)}`} target="_blank" rel="noreferrer" className="group-hover:text-blue-600 group-hover:underline transition-colors" title="Xem trên Google Maps">{selectedUnit.DiaChi}</a>
              ...
              <div className={`space-y-8 transition-all duration-300 ${isSelectedUnitDimmed ? 'opacity-40 pointer-events-none select-none grayscale-[30%]' : ''}`}>
                ...
                <section className="animate-in fade-in duration-500">
                  ...
                  {currentAnNinh ? (
                    <div className="space-y-5 animate-in fade-in duration-300">
                      ...
                      {/* Dòng 1: Cột Lực Lượng (kèm Ca trực) & Cột Giám sát */}
                      <div className="grid grid-cols-1 lg:grid-cols-2 gap-5">
                        ...
                        <div className="bg-white p-5 rounded-xl border border-gray-200 shadow-sm flex flex-col">
                          ...
                          <div className="space-y-3 text-sm flex-1">
                             <div className="flex justify-between"><span className="text-gray-500 font-medium">Tổng ANBV / Định biên:</span><span className="font-black text-[#05469B]">{currentAnNinh.TongANBV || 0} / {currentAnNinh.DinhbienANBV || 0}</span></div>
                             <div className="flex justify-between"><span className="text-gray-500 font-medium">Bảo vệ Nội bộ:</span><span className="font-bold text-gray-800">{currentAnNinh.SoBaoVeNoiBo || 0} người</span></div>
                             <div className="flex justify-between"><span className="text-gray-500 font-medium">Bảo vệ Dịch vụ:</span><span className="font-bold text-gray-800">{currentAnNinh.SoBaoVeDichvu || 0} người</span></div>
                             {Number(currentAnNinh.SoBaoVeDichvu) >= 1 && (
                                <div className="pt-2 border-t border-gray-100 space-y-2">
                                    <div className="flex justify-between"><span className="text-gray-500 font-medium">Vị trí BV Dịch vụ:</span><span className="font-bold text-gray-800 text-right">{currentAnNinh.VitrBVDV || '---'}</span></div>
                                    <div className="flex justify-between"><span className="text-gray-500 font-medium">Nhà cung cấp:</span><span className="font-bold text-gray-800 text-right uppercase">{currentAnNinh.NCC_DichVu || '---'}</span></div>
                                    <div className="flex justify-between"><span className="text-gray-500 font-medium">Chi phí thuê ngoài:</span><span className="font-black text-red-500">{formatCurrency(currentAnNinh.ChiPhiThue) || '0'} VNĐ/tháng</span></div>
                                </div>
                             )}
                          </div>
                          ...
                          <div className="mt-4 pt-4 border-t border-gray-100">
                             <div className="grid grid-cols-2 gap-4">
                                <div className="bg-amber-50/50 p-3 rounded-lg border border-amber-100">
                                   ...
                                   <div className="text-xs text-gray-600 flex justify-between mb-0.5">Cố định: <b>{currentAnNinh.Ngaycd || 0} người</b></div>
                                   <div className="text-xs text-gray-600 flex justify-between">Tuần tra: <b>{currentAnNinh.Ngaytuantra || 0} người</b></div>
                                </div>
                                <div className="bg-indigo-50/50 p-3 rounded-lg border border-indigo-100">
                                   <p className="text-[10px] font-bold text-indigo-800 mb-1 flex items-center gap-1"><Moon size={12}/> CA ĐÊM</p>
                                   <div className="text-xs text-gray-600 flex justify-between mb-0.5">Cố định: <b>{currentAnNinh.Demcd || 0} người</b></div>
                                   <div className="text-xs text-gray-600 flex justify-between">Tuần tra: <b>{currentAnNinh.Demtruantra || 0} người</b></div>
                                </div>
                             </div>
                             
                             {/* TRƯỜNG DỮ LIỆU MỚI: Bố trí nghỉ ca */}
                             <div className="mt-4">
                                <div className="flex items-center gap-3 bg-gradient-to-r from-amber-50 to-indigo-50 px-3 py-2.5 rounded-lg border border-gray-200 shadow-sm">
                                   ...
                                   <span className="font-black text-gray-600 text-xs flex-1 text-right whitespace-pre-wrap break-words">{currentAnNinh.BoTriNghiCa ? `${currentAnNinh.BoTriNghiCa} người` : '---'}</span>
                                </div>
                             </div>
                          </div>
                        </div>
                        ...
                        <div className="bg-white p-5 rounded-xl border border-gray-200 shadow-sm flex flex-col">
                          ...
                          <div className="space-y-3 text-sm">
                             <div className="flex justify-between"><span className="text-gray-500 font-medium whitespace-nowrap shrink-0">Số lượng Camera:</span><span className="font-black text-gray-800 ml-2">{currentAnNinh.SLCAM || 0} Mắt</span></div>
                             <div className="flex justify-between"><span className="text-gray-500 font-medium whitespace-nowrap shrink-0">Thời gian lưu hình:</span><span className="font-bold text-gray-800 ml-2">{currentAnNinh.ThoiGianLuu || '---'} Ngày</span></div>
                             <div className="flex justify-between"><span className="text-gray-500 font-medium whitespace-nowrap shrink-0">Vị trí đặt hệ thống (Đầu ghi):</span><span className="font-bold text-gray-800 text-right ml-2">{currentAnNinh.ViTriDatHeThong || '---'}</span></div>
                             
                             {/* Vị trí giám sát chính - UPDATE KHÔNG RỚT CHỮ CHÍNH */}
                             <div className="flex justify-between items-start pt-2 border-t border-gray-50 mt-1">
                               <span className="text-gray-500 font-medium whitespace-nowrap shrink-0 mr-3">Vị trí giám sát chính:</span>
                               <span className="font-bold text-gray-800 text-right flex-1 whitespace-pre-wrap break-words">{currentAnNinh.ViTriGiamSat || '---'}</span>
                             </div>
                          </div>
                        </div>
                      </div>
                ...
                {/* I. PHỤC VỤ HẬU CẦN (Placeholder) */}
                <section>
                  <div className="flex justify-between items-center mb-5">
                    <h3 className="text-lg font-black text-[#05469B] flex items-center gap-2 uppercase tracking-wider">
                      <div className="w-1.5 h-6 bg-[#05469B] rounded-full"></div> I. PHỤC VỤ HẬU CẦN
                    </h3>
                  </div>
                  <div className="bg-gray-50 hover:bg-amber-50/50 cursor-not-allowed p-8 rounded-xl border-2 border-dashed border-gray-200 text-center transition-all group shadow-sm">
                    <Utensils size={40} className="mx-auto text-gray-300 group-hover:text-amber-400 mb-3 transition-colors" />
                    <h4 className="text-base font-bold text-gray-600 group-hover:text-amber-700 mb-1">Dữ liệu Hậu cần chưa cập nhật</h4>
                    <p className="text-xs text-gray-400 group-hover:text-amber-500">Module này sẽ được mở khóa sau khi cấu hình Sheet dữ liệu hoàn tất.</p>
                  </div>
                </section>
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-5">
                ...
                <div className="md:col-span-3">
                  ...
                  <select required name="ID_DonVi" value={pnFormData.ID_DonVi || ''} onChange={(e) => handleInputChange(e, 'pn')} className="w-full p-2.5 border border-gray-200 rounded-lg bg-[#FFFFF0] outline-none focus:ring-2 focus:ring-orange-500 font-bold text-[#05469B]">
                    {data.filter(dv => selectedUnitSubordinates.includes(dv.ID_DonVi)).map(dv => (
