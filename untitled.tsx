  ...
  useEffect(() => {
    const loadData = async () => {
      ...
      try {
        ...
        // 🚀 BỘ LỌC CHUẨN HÓA DỮ LIỆU TÀI KHOẢN (CHỐNG LỖI UNDEFINED)
        const cleanUsers = rawUsers.map((item: any) => {
          // Ép kiểu mảng nếu Google trả về mảng 2 chiều
          const v = Array.isArray(item) ? item : Object.values(item);
          return {
            ID_User: String(item.ID_User || item.id_user || item.idUser || v[0] || '').trim(),
            Username: String(item.Username || item.username || v[1] || '').trim(),
            Password: String(item.Password || item.password || v[2] || '').trim(),
            HoTen: String(item.HoTen || item.hoTen || v[3] || '').trim(),
            ID_DonVi: String(item.ID_DonVi || item.idDonVi || item.id_donvi || v[4] || '').trim(),
            NhomQuyen: String(item.NhomQuyen || item.nhomQuyen || v[5] || 'USER').trim()
          };
        }).filter((u: any) => u.ID_User && u.ID_User.toUpperCase() !== 'ID_USER'); // Lọc bỏ dòng Tiêu đề của Google Sheets
        ...
        <div className="overflow-x-auto flex-1">
          <table className="w-full text-left">
            ...
            <tbody className="divide-y divide-gray-100">
              ...
              : filteredData.length === 0 ? (<tr><td colSpan={5} className="p-12 text-center text-gray-400"><UserCog size={40} className="mx-auto mb-3 opacity-50"/> Không có dữ liệu.</td></tr>) 
              : filteredData.map(user => (
                <tr key={user.ID_User} className="hover:bg-blue-50/50 transition-colors group">
                  ...
                  <td className="p-4 font-semibold text-gray-700">{donViMap[user.ID_DonVi] || (!user.ID_DonVi ? <span className="text-indigo-600 font-bold px-2 py-1 bg-indigo-50 rounded">TẤT CẢ ĐƠN VỊ (HO)</span> : user.ID_DonVi)}</td>
                  <td className="p-4">
                    <span className={`px-2.5 py-1 rounded-md text-xs font-black border tracking-wider ${user.NhomQuyen === 'ADMIN' ? 'bg-red-50 text-red-600 border-red-200' : 'bg-emerald-50 text-emerald-600 border-emerald-200'}`}>
                      {user.NhomQuyen}
                    </span>
                  </td>
                  <td className="p-4">
